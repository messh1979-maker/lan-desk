...
# --------------------- WAN / Low bandwidth config ---------------------
CONFIG_FILE = "proxy_config.txt"
DEFAULT_PROXY_IP = "192.168.160.149"
PROXY_PORT = 5000

# 256 kbps = about 32 KB/s.
# We target less than line capacity to leave room for TCP, mouse, keyboard, clipboard.
WAN_MODE = True
TARGET_BITRATE_BPS = 200_000

# For 256kbps, 2 FPS often gives better image quality than 5 FPS.
# Local mouse cursor overlay makes it feel more responsive.
FPS = 2
MIN_FPS = 1
MAX_FPS = 5
...
# For text/admin work on 256kbps, grayscale usually looks cleaner.
# If you need color, set it False, but expect more artifacts or lower FPS.
USE_GRAYSCALE = True

# Do not send a frame if screen has not changed significantly.
...
# Cursor visibility
# overlay = draw a synthetic cursor on viewer, recommended
# system  = show normal local mouse cursor
# none    = hide cursor
SHOW_REMOTE_CURSOR = True
CURSOR_MODE = "overlay"

# Mouse packet throttle for WAN.
# The overlay cursor updates immediately, so this only affects remote host packets.
...
# For WAN, do not use very large socket buffers.
# Large buffers increase latency on slow links.
...
def _small_diff(a, b):
    """
    Compare two small grayscale frame thumbnails.
    Returns an approximate number of noticeably changed pixels.
    """
    ...
    for x, y in zip(a, b):
        if abs(x - y) > 8:
            ...
            # Early exit; we only need to know it is above threshold.
...
def run_host(proxy_ip):
    ...
    while True:
        ...
        def scale_mouse_to_host(x, y):
            """
            The viewer may receive a downscaled frame, e.g. 800x600,
            ...
            with mss.mss() as sct:
                ...
                while not stop_event.is_set():
                    ...
                    try:
                        ...
                        # Change detection:
                        # If the screen did not change significantly, do not send a frame.
                        if CHANGE_DETECTION:
                        ...
                        # Optional grayscale for very low bandwidth.
                        if USE_GRAYSCALE:
                        ...
                        # subsampling is only meaningful for RGB JPEG.
                        if img_pil.mode == "RGB":
                        ...
                        if frame_count % 30 == 0:
                            print(
                                f"[Host] Sent {frame_count} frames | q={quality} | "
                                f"fps={current_fps} | size={tw}x{th} | bytes={len(jpeg_data)}"
                            )

                        # Adaptive WAN bitrate control.
                        budget = (TARGET_BITRATE_BPS / 8.0) / max(0.1, current_fps)
                        encoded_size = len(jpeg_data)

                        if encoded_size > budget * 1.15:
                            # Too large for the current bandwidth budget.
                            ...
                            # We have room; improve quality slowly.
