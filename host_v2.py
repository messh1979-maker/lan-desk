...
# -*- coding: utf-8 -*-
"""
remote_host_final.py

Remote Desktop - Host Only
Target OS   : Windows
Proxy IP    : 192.168.160.149
Proxy Port  : 5000

This file runs only as HOST.
It does NOT ask proxy IP from user.

Changes:
- The generated 6-digit connection code is displayed in GREEN.
"""
...
def enable_windows_ansi():
    ...
    Without this, green color may not work on older Windows terminals.
    """
    if sys.platform != "win32":
        return
...
def run_host():
    ...
    print(f"[Host] Using Proxy at {PROXY_IP}:{PROXY_PORT} (hard-coded)")
    ...
    while True:
        ...
        # Connection code is displayed in GREEN.
        print(f"[Host] Your connection code is: {GREEN}{code}{RESET}")

        code_bytes = code.encode()

        try:
            sock.sendall(
                struct.pack("!BI", CMD_HOST_REGISTER, len(code_bytes)) + code_bytes
            )
            resp = sock.recv(1024)
        except Exception as e:
