...
# -*- coding: utf-8 -*-
"""
Remote Desktop Client (Host + Viewer)
Target OS : Windows 10 / 11
Proxy IP  : 192.168.90.250
Proxy Port: 8899
"""
...
# DPI awareness for accurate mouse coordinates on Windows
...
DEFAULT_PROXY_IP = "192.168.90.250"
PROXY_PORT = 8899
...
UNLIMITED_QUALITY = 95
UNLIMITED_FPS = 24
UNLIMITED_JPEG_SUBSAMPLING = 0
...
_KEY_ALIASES = {
    ...
    "linefeed": "enter", "space": "space", "tab": "tab",
    ...
    "sys_req": "printscreen", "pause": "pause",
    "menu": "apps", "apps": "apps",
    "home": "home", "end": "end",
...
_MODIFIER_KEYSYMS = {
    ...
    "Meta_L", "Meta_R", "Win_L", "Win_R",
}

_MOUSE_BUTTONS = {0: "left", 1: "right", 2: "middle"}
...
def send_clipboard_files(send_packet_fn, stop_event, file_paths, transfer_ids):
    ...
    if len(files) > MAX_CLIPBOARD_FILE_COUNT or total > MAX_CLIPBOARD_FILE_TOTAL_SIZE:
        print(f"[Clipboard] Skipping file transfer: too large ({total} bytes, {len(files)} files)")
        return
...
def run_host(proxy_ip):
    ...
    while True:
        ...
        frame_info = {"sent_w": 1920, "sent_h": 1080, "monitor_w": 1920, "monitor_h": 1080}
        bw_ready = threading.Event()
        bw_lock = threading.Lock()
        bw_state = {"target_bps": BW_INITIAL_TARGET_BPS, "estimated_bps": None}
        ...
        def apply_bw_report(seq, measured_bps):
            with bw_lock:
                ...
                bw_state["target_bps"] = clamp(
                    int(estimated * BW_UTILIZATION), BW_MIN_TARGET_BPS, BW_MAX_TARGET_BPS
                )
        ...
        def send_bandwidth_probe(seq, requested_bytes):
            ...
            def _probe():
                try:
                    probe_active.set()
                    total = clamp(int(requested_bytes), BW_MIN_PROBE_BYTES, BW_MAX_PROBE_BYTES)
        ...
        def send_screen():
            ...
            if UNLIMITED_BANDWIDTH_MODE:
                ...
                if initial_target < 700_000:
                    quality = 30; current_fps = 3; max_w = 800; max_h = 600
                elif initial_target < 1_500_000:
                    quality = 45; current_fps = 5; max_w = 1024; max_h = 768
                else:
                    quality = QUALITY; current_fps = FPS
                    max_w = MAX_FRAME_WIDTH; max_h = MAX_FRAME_HEIGHT
                ...
                while not stop_event.is_set():
                    ...
                    try:
                        ...
                        if CHANGE_DETECTION:
                            ...
                            small = img_pil.resize((CHANGE_BLOCK_W, small_h), FAST_RESAMPLE).convert("L")
                            small_bytes = small.tobytes()
                            diff = _small_diff(last_small, small_bytes)
                            if diff < CHANGE_PIXEL_THRESHOLD and (time.monotonic() - last_key_time) < KEYFRAME_INTERVAL:
                        ...
                        buffer = io.BytesIO()
                        save_kwargs = {"format": "JPEG", "quality": int(quality), "optimize": JPEG_OPTIMIZE}
                        if img_pil.mode == "RGB":
                            save_kwargs["subsampling"] = (
                                UNLIMITED_JPEG_SUBSAMPLING if UNLIMITED_BANDWIDTH_MODE else JPEG_SUBSAMPLING
                            )
                        try:
                            img_pil.save(buffer, **save_kwargs)
                        except TypeError:
                            img_pil.save(buffer, format="JPEG", quality=int(quality), optimize=JPEG_OPTIMIZE)
                        ...
                        if frame_count % 30 == 0:
                            print(
                                ...
                                f"q={quality} | fps={current_fps} | size={tw}x{th} | bytes={len(jpeg_data)}"
                            )
            ...
            while not stop_event.is_set():
                try:
                    ...
                    if cmd == CMD_MOUSE_MOVE:
                        ...
                        host_key_press(data.decode(errors="replace"))
                    elif cmd == CMD_KEY_DOWN:
                        host_key_down(data.decode(errors="replace"))
                    elif cmd == CMD_KEY_UP:
                        host_key_up(data.decode(errors="replace"))
                ...
                if CLIPBOARD_FILE_SUPPORT:
                    ...
                    if files:
                        ...
                        if sig != last_file_sig and sig != remote_file_sig:
                            with clip_lock:
                                clip_state["last_file_sig"] = sig
                            send_clipboard_files(send_packet, stop_event, files, clip_file_transfer_ids)
        ...
        t_send.start(); t_recv.start(); t_clip.start()
        t_send.join(); t_recv.join(); t_clip.join()
        try:
...
class ViewerApp:
    def __init__(self, root, proxy_ip):
        ...
        self.connect_btn = tk.Button(
            self.entry_frame, text="Connect", command=self.connect_to_host, font=("Arial", 14)
        )
        ...
        self.view_info = {
            "host_w": 1920, "host_h": 1080,
            "scale_x": 1.0, "scale_y": 1.0,
            "offset_x": 0, "offset_y": 0,
        }
        self.bw_seq = 0
        self.bw_lock = threading.Lock()
        self.bw_active = {"seq": None, "expected": 0, "received": 0, "start": None}
        self.last_bps = 0
        self.bw_thread = None
        initial_clip = get_clipboard_safe()
    ...
    def _update_cursor_overlay(self, wx, wy):
        try:
            ...
            if not self._cursor_items:
                outer = self.canvas.create_oval(wx - 9, wy - 9, wx + 9, wy + 9, outline="white", width=2)
                inner = self.canvas.create_oval(wx - 5, wy - 5, wx + 5, wy + 5, outline="red", width=2)
                self._cursor_items = [outer, inner]
            else:
                outer, inner = self._cursor_items
                self.canvas.coords(outer, wx - 9, wy - 9, wx + 9, wy + 9)
                self.canvas.coords(inner, wx - 5, wy - 5, wx + 5, wy + 5)
    ...
    def connect_to_host(self):
        ...
        try:
            ...
            self.bw_thread = threading.Thread(
                target=self.bandwidth_monitor, args=(self.session_id,), daemon=True
            )
        ...
        while not self.stop_event.is_set() and session_id == self.session_id:
            ...
            if active_seq is not None:
                if start is not None and time.monotonic() - start > BW_PROBE_TIMEOUT:
                    with self.bw_lock:
                        self.bw_active.update({"seq": None, "expected": 0, "received": 0, "start": None})
                else:
                    time.sleep(0.5)
                    continue
            if last > 0:
            ...
            probe_bytes = clamp(probe_bytes, BW_MIN_PROBE_BYTES, BW_MAX_PROBE_BYTES)
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if cmd == CMD_FRAME:
                    ...
                    self._apply_remote_clipboard(data.decode("utf-8", errors="replace"))
                elif cmd == CMD_CLIPBOARD_FILES_BEGIN:
                    ...
                    if len(data) >= 8:
                        ...
                        with self.bw_lock:
                            self.bw_active.update(
                                {"seq": seq, "expected": total, "received": 0, "start": time.monotonic()}
                            )
                        ...
                        with self.bw_lock:
                            if self.bw_active["seq"] == seq:
                                ...
                                if self.bw_active["received"] >= self.bw_active["expected"]:
                                    ...
                                    self.bw_active.update(
                                        {"seq": None, "expected": 0, "received": 0, "start": None}
                                    )
                        if report_seq is not None:
                            print(f"[Viewer] Measured bandwidth: {report_bps} bps")
                            self._send_packet(CMD_BW_REPORT, struct.pack("!II", report_seq, report_bps))
            except Exception as e:
                ...
                if files:
                    ...
                    if sig != last_file_sig and sig != remote_file_sig:
                        ...
                        send_clipboard_files(
                            self._send_packet, self.stop_event, files, self.clip_file_transfer_ids
                        )
    ...
    def _on_button_press(self, event):
        ...
        if btn is None:
            return
        self._send_packet(CMD_MOUSE_BUTTON, struct.pack("!BBII", btn, 1, pos[0], pos[1]))
    ...
    def _on_button_release(self, event):
        ...
        if btn is None:
            return
        self._send_packet(CMD_MOUSE_BUTTON, struct.pack("!BBII", btn, 0, pos[0], pos[1]))
